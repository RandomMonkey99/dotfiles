...
WAYBAR_WEATHER_JSON_TEXT=$(curl "wttr.in/$CITY?m&format=%t")

for ERROR_PREFIX in "${ERROR_PREFIXES[@]}"; do
	if [[ "$WAYBAR_WEATHER_JSON_TEXT" == *"$ERROR_PREFIX"* ]]; then
		WAYBAR_WEATHER_JSON_TEXT="$(ansiweather -l "$CITY" -a false -w false -i false -h false -p false -s false -d false | sed -e 's/[ \t]*Weather.*://g'| tr -d '[:space:]')"
...
if [[ -z ${WAYBAR_WEATHER_DONE} ]]; then
	WAYBAR_WEATHER_JSON_TOOLTIP=$(curl "wttr.in/$CITY?m&format=%t+%C+%c")
	WAYBAR_WEATHER_JSON_CLASS=$(curl "wttr.in/$CITY?m&format=%C")
	WAYBAR_WEATHER_JSON_ICON=$(curl "wttr.in/$CITY?m&format=%c")
